    ...
    public static void initWeekPicker(Context context, final TextView textView, final SmartRefreshLayout refreshLayout) {
        ...
        weekOption = new OptionsPickerView.Builder(context, new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                ...
                mindex = options2;
                windex = options3;
                textView.setText(str1 + "-" + str2 + "-" + str3 + "周");
...
//              final TextView tvAdd = (TextView) v.findViewById(R.id.tv_add);
     ...
     */
    //初始化月报选择器
    public static void initMonthPicker(final Context context, final TextView textView) {
        //控制时间范围(如果不设置范围，则使用默认时间1900-2100年，此段代码可注释)
        //因为系统Calendar的月份是从0-11的,所以如果是调用Calendar的set方法来设置时间,月份的范围也要是从0-11
        final Calendar selectedMonthDate;
     ...
     * 系统日期选择器
     */
    public void CustomDatePickDialog(Context context, final TextView textView, final SmartRefreshLayout refreshLayout) {

        final DatePickerDialog datePickerDialog = new DatePickerDialog(context, null,
                mYear,
                mMonth,
                mDay);
